...
**Chargebee** ships a first-party MCP server. Point Claude, [Claude Code](/posts/2026-05-18-claude-code-vs-cursor-for-production-agent-workflows-in-2026/), or Cursor at it with an API key and the agent can query subscriptions, invoices, usage, entitlements, and upcoming charges in natural language — no REST wrapper, no schema you have to teach the model. In practice this meant our renewal agent could answer "what does this account's next invoice look like and why" in one hop, because Chargebee's data model already thinks in terms of subscriptions, add-ons, and proration. The agent inherits that vocabulary for free. Write operations exist too, but you provision them separately, which is the right default.
