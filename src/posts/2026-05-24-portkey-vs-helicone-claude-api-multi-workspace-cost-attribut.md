...
A workflow with an 80% cache hit rate on a large system prompt looks expensive if cache write tokens are counted at regular input rates (because the occasional write looks like normal input). A workflow that's constantly writing cache — something like [Claude Code](/posts/2026-05-18-claude-code-vs-cursor-for-production-agent-workflows-in-2026/)'s long system prompts refreshing frequently — looks cheaper than it is if cache writes are underweighted.
